...
%      в позицию
%
%  BASELINE_250: частота навигатора возвращена к 250 Гц.
%  ПРИЧИНА: сетка истины 1000 Гц, и 1000/400 = 2.5 не целое. При fnav = 400
%  функция make_increments округляла round(2.5) = 3, из-за чего фактическая
%  частота навигатора составляла 333.3 Гц, а не заявленные 400. Отношение
%  1000/250 = 4 целое, поэтому 250 Гц — корректный проверочный режим.
%  Переход на реальные 400 Гц выполняется отдельным шагом вместе с
%  повышением сетки истины до 2000 Гц (2000/400 = 5).
%
...
% =====================================================================
cfg.fnav       = 250;              % частота навигатора [Гц] (BASELINE_250)
...
g0 = 9.80665;
fprintf('=== ШАГ 8: ESKF НА РЕАЛИСТИЧНОЙ ТРАЕКТОРИИ (BASELINE_250) ===\n');
...
fprintf('%s\n', repmat('-',1,62));
fprintf('\nГоризонтальная ошибка на ударе (КВО-метрика): %.3f м\n', res.err_horiz_final);

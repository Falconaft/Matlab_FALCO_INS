...
%     mc - статистика (та же структура, что у run_montecarlo_eskf)
    ...
    show_progress = isfield(cfg,'show_progress') && cfg.show_progress; % Progres-bar ON
    t_mc = tic;  
...
%=========================================================
        if show_progress
        elapsed = toc(t_mc);
        eta = elapsed/i * (N_mc-i);
    
          fprintf('\rMC [%3d/%3d] %5.1f%% | прошло %.1f мин | осталось ~%.1f мин', ...
          i, N_mc, 100*i/N_mc, elapsed/60, eta/60);

...
%   Вход/выход — см. main_step10_budget.m
    ...
    % ВАЖНО: дальше используется ОТСОРТИРОВАННЫЙ d_std, согласованный
    % с budget.names, иначе имена источников не совпадут с их разбросом.
    d_std = budget.d_std;
    ...
    % =====================================================================
    % Шаг Б.6: ОЦЕНКА ТРЕБУЕМОГО РАЗМЕРА ВЫБОРКИ
    % =====================================================================
    % Разрешающая способность парного сравнения определяется РАЗБРОСОМ
    % парной разности d_std, а не СКО самой выборки.
    %
    % ВАЖНАЯ ОГОВОРКА. Малый d_std означает лишь малый РАЗБРОС парного
    % эффекта, но НЕ обязательно малый сам эффект: источник может давать
    % устойчивое смещение с малой дисперсией. Поэтому источники НЕ делятся
    % на «значимые» и «незначимые» по d_std и НИКАКИЕ источники не
    % исключаются из кампаний.
    %
    % Порог ниже — ВСПОМОГАТЕЛЬНАЯ ЭВРИСТИКА: она нужна только чтобы оценка
    % требуемого N не занижалась источниками с околонулевым разбросом,
    % для которых N не является ограничением. Прежняя версия брала медиану
    % по всем источникам и выдавала заниженное «N >= 4» вместо фактических 29.
    n_signif = sum(budget.signif);
    fprintf('\n  Значимых источников (|t| > 2): %d из %d\n', n_signif, n_src);

    thr_heur = max(0.05 * std_base, 1e-3);
    is_lowvar = d_std < thr_heur;
    sd_use = d_std(~is_lowvar);

    if ~isempty(sd_use)
        sd_med = median(sd_use);
        sd_max = max(sd_use);
        res_now = 2*sd_med/sqrt(N_mc);

        fprintf('  При N = %d разрешимо ~%.2f м\n', N_mc, res_now);
        fprintf('\n  %18s %14s %16s\n', 'разрешить вклад', 'N по медиане', ...
                'N по худшему');
        fprintf('  %s\n', repmat('-',1,50));
        for tgt = [0.5, 0.3, 0.2]
            fprintf('  %15.1f м %14d %16d\n', tgt, ...
                    ceil((2*sd_med/tgt)^2), ceil((2*sd_max/tgt)^2));
        end
        fprintf('  %s\n', repmat('-',1,50));
        fprintf('  СКО парной разности: медиана %.3f м, максимум %.3f м\n', ...
                sd_med, sd_max);
        fprintf('  худший источник: %s\n', budget.names{find(d_std == sd_max, 1)});
        fprintf('  (оценка по источникам с d_std >= %.3f м; источники с меньшим\n', ...
                thr_heur);
        fprintf('   разбросом не ограничивают N и в расчёт не берутся,\n');
        fprintf('   но из кампаний НЕ исключаются)\n');
    end

    budget.d_std_thr_heur = thr_heur;
end
